...
import pandas as pd
from my_utils import read_image, plot_cell_centroid
...
def build_stage_to_morph_from_ome(he0_ome_tif_path: Path) -> np.ndarray:
    """
    Build 3x3 homography: stage(micron) -> morphology pixel (level0)
    using OME-TIFF metadata.
    """
    md = from_tiff(path=str(he0_ome_tif_path))
...
def cells_to_he_pixels(cell_csv_gz: Path, affine_json: Path, he0_ome_tif: Path):
    """
    Read cells.csv.gz (assumed stage micron coords) and convert:
      stage(micron) -> HE0(level0 pixel, morphology_focus) -> HE(level0 pixel)
    using:
      stage_to_morph from OME metadata (HE0 OME-TIFF)
      + he0_to_he_affine_level0.json (affine)
    """
    ...
    # stage(micron) -> he0(level0 px) using OME metadata of morphology_focus
    H_stage2he0 = build_stage_to_morph_from_ome(he0_ome_tif)  # 3x3
    xy_he0 = transform_coordinates(xy_stage, H_stage2he0)
    print(f"[DEBUG] xy_he0(level0 px) min={xy_he0.min(axis=0)} max={xy_he0.max(axis=0)}", flush=True)

    # # he0(level0 px) -> he(level0 px) using affine
    # A2, _ = load_affine(affine_json)
    # ones = np.ones((xy_he0.shape[0], 1), dtype=np.float32)
    # xy1 = np.concatenate([xy_he0.astype(np.float32), ones], axis=1)   # (N,3)
    # xy_he = (xy1 @ A2.T).astype(np.float32)                            # (N,2)
    # print(f"[DEBUG] xy_he(level0 px) min={xy_he.min(axis=0)} max={xy_he.max(axis=0)}", flush=True)

    # he0(level0 px) -> he(level0 px) using homography
    _, H3 = load_homography(affine_json)
    xy = xy_he0.astype(np.float32)  # (N,2)
    ones = np.ones((xy.shape[0], 1), dtype=np.float32)
...
class FinalAlignmentApp(tk.Tk):
    def __init__(self, run_dir: Path):
        ...
        self.case_id = int(self.info.get("HE0_orientation_case", 0))
        self.display_level = int(self.info["HE0_level"])
        ...
        self.cache = {
            "he0_base": self.run_dir / f"9_cache_he0_base_L{self.display_level}.png",
            "he_base": self.run_dir / f"9_cache_he_base_L{self.display_level}.png",

            "he0_kp": self.run_dir / f"9_cache_he0_kp_L{self.display_level}.png",
        ...
        self.H3 = None
        self.warped_he0 = None
        self.he_he0_overlay = None

        self.he0_pts0 = None
        self.he_pts0 = None

        # -------- load base images ONLY (no alignment) --------
        self.he0_rgb, self.he0_img = self._load_or_build_he0_base()
        self.he_rgb, self.he16 = self._load_or_build_he_base()
        print("[DEBUG] HE0 image shape:", self.he0_rgb.shape, flush=True)
        ...
        tk.Button(
            btns, text="Export registered HE0 (OME-TIFF)",
            command=self.export_registered_he0_ome
        ).pack(side="left", expand=True, fill="x")

        tk.Button(
            btns, text="Export HE0 registered (L0 uint16 OME-TIFF)",
            command=lambda: self.export_registered_he0_L0_uint16(tile=1024)
        ).pack(side="left", expand=True, fill="x")
    ...
    def refresh_images_initial(self):
        # panel1/2 base
        self._set_panel(0, apply_orientation_case(self.he0_rgb, self.case_id))
    ...
    def refresh_images_after_alignment(self):
        # panel0: HE0 kp (prefer cache)
        p = self.cache["he0_kp"]
        he0_kp = self._imread(p)
        if he0_kp is None:
            pts = self.he0_pts0 / self.display_scale
            he0_kp = draw_points(self.he0_rgb, pts, color=(0, 255, 0), r=6, name="HE0 nuclei")
            self._imsave(p, he0_kp)
        self._set_panel(0, apply_orientation_case(he0_kp, self.case_id))
    ...
    def load_alignment_and_keypoints(self):
        ...
        def build_tps_remap_for_display(tps_model, out_shape_hw, display_scale):
            ...
            ----------
            tps_model : dict
                inverse TPS model: HE(level0) -> HE0(level0)
            out_shape_hw : (H, W)
            ...
            # inverse TPS: HE(level0) -> HE0(level0)
            pts_he0_lvl0 = apply_tps(pts_he_lvl0, tps_model)

            # level0 HE0 px -> display HE0 px
            pts_he0_disp = pts_he0_lvl0 / s

            map_x = pts_he0_disp[:, 0].reshape(H, W).astype(np.float32)
            map_y = pts_he0_disp[:, 1].reshape(H, W).astype(np.float32)
        ...
        try:
            ...
            self.he0_pts0 = np.array([x["he0_centroid_global"] for x in nuclei], np.float32)
            self.he_pts0 = np.array([x["he_centroid_global"] for x in nuclei], np.float32)
            print(f"[INFO] Loaded nuclei: {len(self.he0_pts0)}", flush=True)

            # -------- load transform --------
            tf_path = self.run_dir / "he0_to_he_homography_level0.json"
            ...
            if mode == "matrix":
                ...
                self.warped_he0 = cv2.warpPerspective(
                    self.he0_rgb,
                    H_disp,
                ...
                # TPS path: for image warp we must use inverse TPS (HE -> HE0)
            ...
            # -------- overlay --------
            alpha = 0.5  # HE0 transparency
            self.he_he0_overlay = cv2.addWeighted(
                self.he_rgb,  # background (HE)
                1.0 - alpha,  # weight of HE
                self.warped_he0,  # overlay (HE0)
                alpha,  # weight of HE0
                0
            )
            cv2.imwrite(str(self.cache["overlay"]), self.he_he0_overlay)
            ...
            # -------- warp + overlay --------
            warped_he0_manual = cv2.warpPerspective(
                self.he0_rgb,
                Hm_disp,
                (self.he_rgb.shape[1], self.he_rgb.shape[0]),
                flags=cv2.INTER_LINEAR,
            ...
            he_he0_overlay_manual = cv2.addWeighted(
                self.he_rgb,
                1.0 - alpha,
                warped_he0_manual,
                alpha,
            ...
            cv2.imwrite(str(self.cache["manual"]), he_he0_overlay_manual)

            img1_w = add_watermark(Image.fromarray(self.he_he0_overlay[..., ::-1]), "Final alignment")
            img2_w = add_watermark(Image.fromarray(he_he0_overlay_manual[..., ::-1]), "Manual initial alignment")
            img1_w.save(
            ...
            # 1) HE0 kp
            p_he0_kp = self.cache["he0_kp"]
            he0_kp = self._imread(p_he0_kp)
            if he0_kp is None:
                pts = self.he0_pts0 / self.display_scale
                he0_kp = draw_points(self.he0_rgb, pts, color=(0, 255, 0), r=6, name="HE0 nuclei")
                self._imsave(p_he0_kp, he0_kp)
            ...
            self._panel3_tkimg_he = self._make_tkimg(self.he_rgb)
            self._panel3_tkimg_overlay = self._make_tkimg(self.he_he0_overlay)
            self._panel3_show_overlay = True
    ...
    def export_registered_he0_ome(self):
        """
        Export HE0 warped into HE space at DISPLAY_LEVEL as an OME-TIFF.
        Output: <run_dir>/9_he0_registered_L{DISPLAY_LEVEL}.ome.tif
        """
        if self.warped_he0 is None:
            messagebox.showinfo(
                "Not available",
                "No warped HE0 in memory yet.\nClick 'Load keypoints + alignment matrix' first.",
                parent=self
            )
            return

        # If you want 16-bit original intensity warped, that is a different export (full-res).
        out_path = self.run_dir / f"9_he0_registered_L{self.display_level}.ome.tif"
        ...
        tf.imwrite(
            str(out_path),
            self.warped_he0[..., ::-1],  # BGR->RGB for writing
            photometric="rgb",
            compression="lzw",
            metadata=meta
        )
    ...
    def export_registered_he0_L0_uint16(self, tile=1024):
        """
        Read HE0 L0 (uint16) -> warp to HE L0 space using he0_to_he_homography_level0.json -> save OME-TIFF.
        Output: <run_dir>/9_he0_registered_L0.ome.tif

        Notes:
        - Uses zarr window reads from HE0 OME-TIFF pyramid: root["0"].
        ...
        H_path = self.run_dir / "he0_to_he_homography_level0.json"
        out_path = self.run_dir / "9_he0_registered_L0.ome.tif"
        ...
        # ---- open HE0 L0 via zarr ----
        store = tf.imread(he0_path, aszarr=True)
        root = zarr.open(store, mode="r")
        if "0" not in root:
            raise ValueError(f"HE0 zarr root has no '0'. keys={list(root.keys())[:20]}")
        z = root["0"]  # L0 uint16
        HE0_H, HE0_W = z.shape[:2]
        print("[EXPORT L0] HE0 L0:", HE0_W, HE0_H, "dtype:", z.dtype, flush=True)

        # ---- helper: bbox in HE0 for a target HE tile ----
        ...
        out_path.parent.mkdir(parents=True, exist_ok=True)
        tmp_raw = Path(tempfile.mkstemp(prefix="he0_reg_L0_", suffix=".dat")[1])
    ...
    def _load_or_build_he0_base(self):
        img = self._imread(self.cache["he0_base"])
        if img is not None:
            return img, None

        he0_img, _ = read_image(
            self.info["HE0_path"],
            keep_16bit=False,
        ...
        if he0_img.ndim == 2:
            bgr = cv2.cvtColor(he0_img.astype(np.uint8), cv2.COLOR_GRAY2BGR)
        else:
            if he0_img.dtype != np.uint8:
                he0_img = np.clip(he0_img / 256.0, 0, 255).astype(np.uint8)
            bgr = cv2.cvtColor(he0_img, cv2.COLOR_RGB2BGR)

        self._imsave(self.cache["he0_base"], bgr)
        return bgr, he0_img
    ...
    def _update_panel3_fast(self):
        if self.he_he0_overlay is None:
    ...
    def load_cell_data(self):
        ...
        # --- load & transform ---
        try:
            # affine_json = self.run_dir / "he0_to_he_affine_level0.json"
            perspective_json = self.run_dir / "he0_to_he_homography_level0.json"
            df = cells_to_he_pixels(
                path,
                affine_json=perspective_json,
                he0_ome_tif=Path(self.info["HE0_path"]),
            )
    ...
    def on_panel_click(self, idx: int):
        ...
        # panel 0: HE0
        if idx == 0:
            if self.alignment_loaded and self.he0_pts0 is not None:
                pts = self.he0_pts0 / self.display_scale
                img = draw_points(self.he0_rgb, pts, color=(0, 255, 0), r=6, name="HE0 nuclei (large)")
            else:
                img = self.he0_rgb.copy()
            img = apply_orientation_case(img, self.case_id)
            self.show_large_view("HE0", img)
            return
        ...
        if idx == 2:
            if self.he_he0_overlay is None:
                ...
                return
            img = self.he_he0_overlay if self._panel3_show_overlay else self.he_rgb
