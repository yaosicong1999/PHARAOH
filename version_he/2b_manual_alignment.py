...
class ManualAlignView(QGraphicsView):
    """
    支持两套底图/overlay：
      - mask 模式：he_mask_pix / he0_mask_pix
      - original 模式：he_orig_pix / he0_orig_pix

    swap 时记录当前 overlay pose，并把 pose 应用到另一套 pixmap 上。
    """
    MODE_MASK = "mask"
    MODE_ORIG = "original"
    def __init__(self, he_mask_path, he0_mask_path, he_orig_path, he0_orig_path, case_id):
        super().__init__()
    ...
    def fit_bg_to_view(self):
        ...
        pad_x = bg_rect.width() * 0.5
        pad_y = bg_rect.height() * 0.5
    ...
    def get_corners_he0_to_he(self):
        ...
        顺序: TL, TR, BL, BR
        """
        rect = self.overlay.boundingRect()  # HE0 local 坐标系里的矩形
    ...
    def apply_homography_to_overlay(self, H_gui2he: np.ndarray):
        """
        Apply a 3x3 homography that maps overlay-local coords (GUI HE0 pixels)
...
class ManualAlignWindow(QWidget):
    def __init__(self, run_dir, he_mask_path, he0_mask_path, he_orig_path, he0_orig_path,
                 he0_gui_affine, case_id=0, info=None):
        super().__init__()
        ...
        self.view = ManualAlignView(he_mask_path, he0_mask_path, he_orig_path, he0_orig_path, case_id=case_id)
    ...
    def on_save(self):
        ...
        Ld = int(self.info.get("HE0_level", 0))  # 你需要在 images_info.json 里明确写出来
        ...
        try:
            ...
            he0_rgba = mask_to_rgba(warped_he0_mask, color_rgb=(0, 0, 255), alpha=0.5)  # 蓝
            ...
            he0_raw_bgr = cv2.imread(str(run_dir / "1_he0_threshold_mask.png"), cv2.IMREAD_COLOR)
...
if __name__ == "__main__":
    ...
    he0_orig_path = run_dir / "1_he0_threshold_mask.png"
