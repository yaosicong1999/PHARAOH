# ============================================================
# 4c_get_nuclei_patches.py - Stage 4c: Nucleus patch extraction
#
# Crops HE0/HE patches around each standout nucleus and writes
# nuclei_patches/nuclei_centroids_global.json (gates Stages 8-9).
# Usage: python 4c_get_nuclei_patches.py <TILES_DIR>
# ============================================================
import json
import cv2
import os
...
def extract_level_to_read_level(extract_level: int) -> int:
    """
    convert stage3 extract level convention to read_image level convention:
      extract 1 -> read 0
      extract 2 -> read 1
      extract 3 -> read 2
      ...
...
def process_he0(image_file, upscale=2, n_smooth=2, intensity_threshold=0.6, dot_r=4):
    try:
...
def process_he(image_file, upscale=2, n_smooth=2, intensity_threshold=0.6, dot_r=4):
    try:
        rgb_tile = np.array(Image.open(image_file))
        labeled_mask, mask_dark = segment_super_dark_nuclei_full(
            rgb_tile, upsample_scale=upscale, n_smooth=int(n_smooth), intensity_threshold=float(intensity_threshold)
        )
...
def main(run_dir):
    ...
    HE0_PATCH_LEN = int(parameters.get("stage4c", {}).get("he0_patch_len", 200))
    ...
    # stage3 extract level -> actual read_image level
    ...
    stage4c = parameters.get("stage4c", {}) if isinstance(parameters, dict) else {}
    he0_upscale = int(stage4c.get("he0_mask_upscale_factor", 2))
    he0_n_smooth = int(stage4c.get("he0_mask_n_smooth", 2))
    he_upscale = int(stage4c.get("he_mask_upscale_factor", 2))
    he_n_smooth = int(stage4c.get("he_mask_n_smooth", 2))
    # intensity thresholds: parameters.json defaults, overridden by pilot if present
    pilot_he0_thr, pilot_he_thr = load_pilot_mask_params(
        out_dir,
        default_he0_thr=float(stage4c.get("he0_mask_intensity_threshold", 0.6)),
        default_he_thr=float(stage4c.get("he_mask_intensity_threshold", 0.6)),
    )
    out_dir = Path(out_dir)
    for f in glob(str(out_dir / "*_he0_patch.png")):
        print(process_he0(f, upscale=he0_upscale, n_smooth=he0_n_smooth, intensity_threshold=pilot_he0_thr))
    for f in glob(str(out_dir / "*_he_patch.png")):
        print(process_he(f, upscale=he_upscale, n_smooth=he_n_smooth, intensity_threshold=pilot_he_thr))
