# ============================================================
# 4_tile_gallery.py - Stage 4: Tile gallery & nuclei orchestration
#
# Tk gallery over extracted tiles; runs 4a (nuclei masks) ->
# 4b (standout nuclei) -> 4c (nucleus patches) as subprocesses.
# Usage: python 4_tile_gallery.py <RUN_DIR>
# ============================================================
...
def show_tile_gallery_in_memory(
    ...
    assert len(he0_tiles) == len(he_tiles)
    ...
    root.title("Stage 4: H&E-FG & H&E Tile Gallery")
    idx = [0]
    # ---------- Labels ----------
    tk.Label(root, text="H&E-FG", font=("Helvetica", 15)).grid(row=0, column=0)
    tk.Label(root, text="H&E-FG nuclei mask", font=("Helvetica", 15)).grid(row=0, column=1)
    ...
    tile_bases = []
    for d in he0_tiles:
        base = d["filename"].replace("_he.png", "")
    ...
    def refresh_button_states(running=False):
        ...
        s5 = stage4a_done_by_files()
        s6 = stage4b_done_by_files()
        s7 = stage4c_done_by_files()

        # allow re-run stage4a anytime
        btn_run5.config(state="normal")
        # stage4b only if stage4a done
        btn_run6.config(state=("normal" if s5 else "disabled"))
        # stage4c only if stage4b done
    ...
    def launch_script_with_progress(root, script_name, title):
        log_event(run_dir, "user_click_run_substage", script=script_name)
        ...
        try:
            ...
            for line in proc.stdout:
                ...
                if line.startswith("[INFO]"):
                    ...
                    log_event(run_dir, "system_ready_run_substage", script=script_name)
                    messagebox.showinfo(
                        "Done",
                    ...
                    if script_name == "4c_get_nuclei_patches.py":
                        messagebox.showinfo(
                            "Stage 4c Finished",
                            "Nucleus patch extracted.\nYou can open the nuclei patch gallery manually (Stage 5)."
                        )
                    break
    ...
    def update_images():
        ...
        # ----------------------------
        base = he0["filename"].replace("_he0.png", "")
        he0_img_name = he0["filename"]
        ...
        he0_pil = load_optional(
            os.path.join(output_folder, he0_img_name),
            is_mask=False,
            case_id=case_id
        )
        ...
        # HE0 mask
        # ----------------------------
        he0_mask_pil = load_optional(
            os.path.join(output_folder, f"{base}_he0_mask.png"),
            is_mask=True,
            case_id=case_id
        )
        ...
        info_label.config(
            text=f"{i + 1}/{len(he0_tiles)} | {he0['type'].capitalize()}"
        )

    def next_tile():
        idx[0] = (idx[0] + 1) % len(he0_tiles)
        update_images()
...
def main():
    ...
    with open(os.path.join(output_folder, "he0_tile_info.json"), "r") as f:
    ...
    he0_tiles = [he0_info[k] for k in keys]
