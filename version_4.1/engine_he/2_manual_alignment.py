# ============================================================
# 2_manual_alignment.py - Stage 2: Manual initial alignment
#
# Interactive HE0->H&E overlay (affine / perspective handles) to
# produce an initial homography, saved to
# manual_initial_alignment.json along with overlay previews.
# Usage: python 2_manual_alignment.py <RUN_DIR>
# ============================================================
import math
import numpy as np
import cv2
...
from PyQt5.QtCore import Qt, QPointF, QRectF, QEvent, QThread, pyqtSignal
...
print("[Stage2] imports done", flush=True)

# -----------------------------
# Utilities
# -----------------------------
def log_event(run_dir, event_name, stage="stage2", **extra):
...
class HE0OverlayItem(QGraphicsPixmapItem):
    MODE_AFFINE = "affine"
...
class ManualAlignView(QGraphicsView):
    """
    Supports two background/overlay modes:
      - mask mode: he_mask_pix / he0_mask_pix
      - original mode: he_orig_pix / he0_orig_pix

    When switching modes, the current overlay pose is preserved
    and applied to the other pixmap set.
    """
    MODE_MASK = "mask"
    MODE_ORIG = "original"
    # -------------------------
    # init / view setup
    # -------------------------
    def __init__(self, he_mask_path, he0_mask_path, he_orig_path, he0_orig_path):
        super().__init__()
        ...
        self.he0_mask_pix = mask_to_colored_pixmap(
            he0_mask_path,
        ...
        self.he0_orig_pix = QPixmap(he0_orig_path)

        # ---- items ----
        self.he_item = QGraphicsPixmapItem(self.he_orig_pix)
        self.scene.addItem(self.he_item)
        self.overlay = HE0OverlayItem(self.he0_orig_pix)
        ...
        self.view_pad = 0.95
        self.view_pad_min = 0.2
        self.view_pad_max = 1.8
        self.view_pad_step = 0.05
    ...
    def fit_bg_to_view(self):
        bg_rect = self.he_item.boundingRect()

        pad_x = bg_rect.width() * 0.2
        pad_y = bg_rect.height() * 0.2

        scene_rect = QRectF(
            bg_rect.left() - pad_x,
            bg_rect.top() - pad_y,
            bg_rect.width() + 2 * pad_x,
            bg_rect.height() + 2 * pad_y,
        )
        ...
        pad = getattr(self, "view_pad", 0.95)
        self.scale(pad, pad)

    def adjust_view_pad(self, delta):
        old_pad = self.view_pad
        self.view_pad = max(self.view_pad_min, min(self.view_pad_max, self.view_pad + delta))
        print(f"[zoom] view_pad: {old_pad:.2f} -> {self.view_pad:.2f}", flush=True)
        self.fit_bg_to_view()
    ...
    def keyPressEvent(self, event):
        ...
        # macOS Command key = MetaModifier
        is_cmd = bool(mods & Qt.MetaModifier)

        if is_cmd and event.key() in (Qt.Key_Plus, Qt.Key_Equal):
            self.adjust_view_pad(+self.view_pad_step)
            event.accept()
            return

        if is_cmd and event.key() in (Qt.Key_Minus, Qt.Key_Underscore):
            self.adjust_view_pad(-self.view_pad_step)
            event.accept()
            return
    ...
    # -------------------------
    # overlay pose helpers
    # -------------------------
    def init_overlay_pose(self, scale=0.35):
        self.overlay.enter_affine_mode()
    ...
    def get_overlay_pose(self):
        """Return the current overlay pose (must include pos/rotation/transform)."""
        return {
    ...
    def apply_overlay_pose_keep_scene_center(self, pose):
        """
        Apply a pose to the overlay while keeping its scene center unchanged,
        preventing drift after swapping pixmaps.
        """
        ...
        # compute new center and compensate shift
    ...
    def swap_mode(self):
        """
        Core logic when swapping modes:
          1) Record current overlay pose
          2) Switch background + overlay pixmap
          3) Apply pose to the new pixmap (with center compensation)
        """
        clear_override_cursor()
        ...
        if self.mode == self.MODE_MASK:
            ...
            self.overlay.setPixmap(self.he0_orig_pix)
            ...
            self.overlay.setPixmap(self.he0_mask_pix)
            self.apply_overlay_pose_keep_scene_center(pose)
            self.overlay.reset_perspective_cache()
    ...
    def get_corners_he0_to_he(self):
        """
        Return 4 corner correspondences:
          src = HE0 local (pixel) corners
          dst = HE pixel corners (scene coordinates)
        Order: TL, TR, BR, BL
        """
        rect = self.overlay.boundingRect()  # HE0 local coordinate rectangle
    ...
    def apply_homography_to_overlay(self, H_gui2he: np.ndarray):
        """
        Apply a 3x3 homography mapping overlay-local coordinates (GUI HE0 pixels)
        directly to scene coordinates (HE pixels).

        NOTE: This sets the overlay pose immediately (rotation=0, pos=(0,0), transform=H).
        """
        ...
        # reset pose components to avoid double transformation
        self.overlay.setRotation(0)
        self.overlay.setPos(0, 0)
    ...
    def _set_overlay_hidden(self, hide: bool):
        """Unified entry to toggle overlay visibility (avoid redundant setVisible calls)."""
    ...
    def eventFilter(self, obj, event):
        ...
        # fallback: sometimes Meta key press/release is unreliable,
        # so also check via modifiers
        def meta_down(e):
            try:
        ...
        if et == QEvent.KeyPress:
            # Command pressed (or any key while Meta is held)
            if event.key() == Qt.Key_Meta or meta_down(event):
                self._set_overlay_hidden(True)
                return False  # do not consume (avoid blocking Cmd+Q / Cmd+W)
            ...
            # fallback: if other key released but Meta no longer held, restore
            if self._overlay_hidden_by_key and not meta_down(event):
                self._set_overlay_hidden(False)
                return False

        elif et in (QEvent.FocusOut, QEvent.WindowDeactivate):
            # restore if window loses focus (avoid stuck hidden state)
            if self._overlay_hidden_by_key:
                self._set_overlay_hidden(False)
...
# -----------------------------
# Background ORB auto-alignment worker
# -----------------------------
class OrbAlignWorker(QThread):
    """
    Runs a rough ORB feature registration HE0 -> H&E in the background, on the
    same downsampled low-level images the GUI loads (1_he0_level_image.png /
    1_he_level_image.png). Emits a 3x3 homography that maps GUI-HE0 pixels ->
    HE scene pixels (directly usable by
    ManualAlignView.apply_homography_to_overlay), plus the RANSAC inlier count.

    Both images are brightfield H&E (same modality), so ORB tends to match
    well; the inlier count is still surfaced so the user can judge quality
    before applying.
    """
    finished_ok = pyqtSignal(object, int)   # (H_3x3 float64, n_inliers)
    failed = pyqtSignal(str)

    def __init__(self, src_path, dst_path, invert_src=False, max_dim=1600,
                 n_features=8000, ratio=0.75, ransac_thresh=5.0, parent=None):
        super().__init__(parent)
        self.src_path = str(src_path)   # overlay image (HE0)
        self.dst_path = str(dst_path)   # background image (HE)
        self.invert_src = bool(invert_src)
    ...
    def _scale_for(self, img):
        h, w = img.shape[:2]
        return min(1.0, self.max_dim / float(max(h, w)))

    def run(self):
        try:
            src = cv2.imread(self.src_path, cv2.IMREAD_COLOR)
            dst = cv2.imread(self.dst_path, cv2.IMREAD_COLOR)
            if src is None or dst is None:
                self.failed.emit("could not read H&E-FG/H&E low-level images")
                return

            ss, sdst = self._scale_for(src), self._scale_for(dst)
            src_s = cv2.resize(src, None, fx=ss, fy=ss, interpolation=cv2.INTER_AREA) if ss < 1 else src
            dst_s = cv2.resize(dst, None, fx=sdst, fy=sdst, interpolation=cv2.INTER_AREA) if sdst < 1 else dst

            g_src = self._prep(src_s, invert=self.invert_src)
            g_dst = self._prep(dst_s, invert=False)
            ...
            if d1 is None or d2 is None or len(k1) < 4 or len(k2) < 4:
                self.failed.emit("too few ORB features detected")
                return
            ...
            if len(good) < 4:
                self.failed.emit(f"only {len(good)} good matches (need >=4)")
                return
            ...
            H_s, mask = cv2.findHomography(pts_src, pts_dst, cv2.RANSAC, self.ransac_thresh)
            if H_s is None:
                self.failed.emit("findHomography returned None")
                return
            n_inl = int(mask.sum()) if mask is not None else 0

            # H_s maps scaled-src -> scaled-dst; lift back to full-res file pixels
            # (which are exactly the GUI pixmap coords): H = S_dst^-1 @ H_s @ S_src
            S_src = np.diag([ss, ss, 1.0])
            S_dst_inv = np.diag([1.0 / sdst, 1.0 / sdst, 1.0])
            H_full = S_dst_inv @ H_s.astype(np.float64) @ S_src
            if abs(H_full[2, 2]) > 1e-12:
                H_full = H_full / H_full[2, 2]
            self.finished_ok.emit(H_full, n_inl)
...
class ManualAlignWindow(QWidget):
    def __init__(self, run_dir, he_mask_path, he0_mask_path, he_orig_path, he0_orig_path,
                 he0_gui_affine, info=None):
        super().__init__()
        self.run_dir = str(run_dir)
        self.info = info or {}
        self.he0_gui_affine = np.asarray(he0_gui_affine, dtype=np.float32)

        # keep the low-level image paths for background ORB registration
        self.he_orig_path = str(he_orig_path)
        self.he0_orig_path = str(he0_orig_path)

        self.external_h_mat = None
        self.external_h_source = None

        # auto-align (ORB) state
        self.auto_H = None
        self.orb_worker = None

        self.setWindowTitle("Manual Alignment (auto ORB)")

        self.view = ManualAlignView(he_mask_path, he0_mask_path, he_orig_path, he0_orig_path)
        self.resize(1400, 900)

        self.btn_load_h = QPushButton("Load H (.json)")
        self.btn_auto   = QPushButton("Auto-align (ORB): computing…")
        ...
        self.btn_auto.clicked.connect(self.on_apply_auto)
        ...
        # kick off rough ORB registration in the background; the user can keep
        # aligning by hand while this runs, then click the button to apply it.
        self._start_auto_align()

    # -------------------------
    # ORB auto-alignment (background suggestion)
    # -------------------------
    def _start_auto_align(self):
        # HE0 -> HE, both brightfield H&E: same polarity, so no invert.
        self.orb_worker = OrbAlignWorker(self.he0_orig_path, self.he_orig_path,
                                         invert_src=False, parent=self)
        self.orb_worker.finished_ok.connect(self.on_auto_ready)
        self.orb_worker.failed.connect(self.on_auto_failed)
        self.orb_worker.start()
        log_event(self.run_dir, "system_start_orb_autoalign")

    def on_auto_ready(self, H, n_inl):
        self.auto_H = np.asarray(H, dtype=np.float64)
        self.btn_auto.setText(f"Apply Auto-align (ORB · {n_inl} inliers)")
        self.btn_auto.setEnabled(True)
        print(f"[Stage2][ORB] ready: {n_inl} inliers", flush=True)
        log_event(self.run_dir, "system_ready_orb_autoalign", inliers=int(n_inl))

    def on_auto_failed(self, msg):
        self.auto_H = None
        self.btn_auto.setText("Auto-align (ORB): unavailable")
        self.btn_auto.setEnabled(False)
        print(f"[Stage2][ORB] failed: {msg}", flush=True)
        log_event(self.run_dir, "system_failed_orb_autoalign", reason=str(msg))

    def on_apply_auto(self):
        if self.auto_H is None:
            return
        # The ORB homography maps HE0-image pixels -> HE-image pixels, i.e. the
        # ORIGINAL-mode pixmaps. Switch to original mode so overlay-local coords
        # match what apply_homography_to_overlay expects.
        if self.view.mode != self.view.MODE_ORIG:
            self.view.swap_mode()
        self.view.apply_homography_to_overlay(self.auto_H)
        self.pose_orig = self.view.get_overlay_pose()
        log_event(self.run_dir, "user_click_apply_orb_autoalign")
    ...
    def on_load_h_json(self):
        ...
        try:
            ...
            A = np.asarray(self.he0_gui_affine, dtype=np.float64)
            if A.shape == (2, 3):
                A = np.vstack([A, [0, 0, 1]]).astype(np.float64)
            if A.shape != (3, 3):
                raise ValueError(f"he0_gui_affine must be (2,3) or (3,3), got {A.shape}")
            ...
            QMessageBox.information(
                ...
                f"H shape: {H.shape}\n"
                f"The homography has been applied to the current overlay."
            )
        except Exception as e:
    ...
    def on_save(self):
        log_event(self.run_dir, "user_click_save_alignment")
        cur_pose = self.view.get_overlay_pose()
        if self.view.mode == self.view.MODE_MASK:
            self.pose_mask = cur_pose
        else:
        ...
        # 1) Corner correspondences: GUI HE0 -> HE
        src_gui, dst_he = self.view.get_corners_he0_to_he()  # (4,2) float32
        # 2) Convert GUI coordinates back to original HE0 coordinates
        T_gui = np.array(self.he0_gui_affine, dtype=np.float32)  # original -> GUI
        if T_gui.shape == (2, 3):
        ...
        # 3) Estimate homography: original HE0 -> HE
        ...
        Ld = int(self.info.get("HE0_level", 0))
        Lh = int(self.info.get("HE_level", 0))
        sd = float(2 ** Ld)
        sh = float(2 ** Lh)
        S_d = np.array([[sd, 0, 0],
                        [0, sd, 0],
        ...
        print("Saved manual_initial_alignment.json")
        ...
        try:
            ...
            # Original-space overlay preview (the mask overlay was removed).
            src_gui, dst_he = self.view.get_corners_he0_to_he()
            H_gui2he = cv2.getPerspectiveTransform(
                np.asarray(src_gui, dtype=np.float32),
                np.asarray(dst_he, dtype=np.float32),
            ).astype(np.float32)
            he_orig_bgr = cv2.imread(str(run_dir / "1_he_level_image.png"), cv2.IMREAD_COLOR)
            he0_raw_bgr = cv2.imread(str(run_dir / "1_he0_level_image.png"), cv2.IMREAD_COLOR)
            if he_orig_bgr is None or he0_raw_bgr is None:
                raise RuntimeError("Failed to read original images for overlay saving.")
            warped = cv2.warpPerspective(
                he0_raw_bgr, _as_3x3(H_gui2he),
                (he_orig_bgr.shape[1], he_orig_bgr.shape[0]),
                flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT,
            )
            out = cv2.addWeighted(he_orig_bgr, 0.7, warped, 0.8, 0)
            cv2.imwrite(str(run_dir / "2_manual_overlay_original.png"), out)
            print("[INFO] Saved overlay:", str(run_dir / "2_manual_overlay_original.png"), flush=True)

        except Exception as e:
            print(f"[WARN] overlay save failed: {e}", flush=True)

        log_event(self.run_dir, "system_ready_save_alignment")
        ...
        log_event(self.run_dir, "system_ready_exit")
        QApplication.quit()
...
if __name__ == "__main__":
    ...
    he0_mask_path = run_dir / "1_confirmed_he0_dense_mask.png"
    he_orig_path   = run_dir / "1_he_level_image.png"
    he0_orig_path = run_dir / "1_he0_level_image.png"

    # The mask overlay view was removed; only the original level images are
    # required. Fall back to the originals for the (unused) mask pixmaps so the
    # view still constructs if the old mask files are absent.
    missing = [p for p in [he_orig_path, he0_orig_path] if not p.exists()]
    if missing:
    ...
    if not he_mask_path.exists():
        he_mask_path = he_orig_path
    if not he0_mask_path.exists():
        he0_mask_path = he0_orig_path

    if "HE0_gui_affine" not in info:
        print("[ERROR] HE0_gui_affine missing in images_info.json")
        sys.exit(2)
    he0_gui_affine = np.array(info["HE0_gui_affine"], dtype=np.float32)

    app = QApplication(sys.argv)
    window = ManualAlignWindow(
        run_dir=str(run_dir),
        he_mask_path=str(he_mask_path),
        he0_mask_path=str(he0_mask_path),
        he_orig_path=str(he_orig_path),
        he0_orig_path=str(he0_orig_path),
        he0_gui_affine=he0_gui_affine,
        info=info
    )
    window.show()
