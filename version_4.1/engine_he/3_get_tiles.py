# ============================================================
# 3_get_tiles.py - Stage 3: Tile extraction
#
# Builds an HE0 "available region" from the HE0 hematoxylin
# threshold mask, samples tile centroids (CVT), then crops aligned
# HE0 + HE tiles into <RUN_DIR>/tiles/ (he0_tile_info.json /
# he_tile_info.json).
# Usage: python 3_get_tiles.py <RUN_DIR>
# ============================================================
import os
import sys
import json
...
def load_stage3_params(script_path: Path):
    ...
    _blk = cli_params_block()
    if isinstance(_blk.get("stage3", {}), dict):
        stage3.update(_blk.get("stage3", {}))
...
def make_available_mask_from_he0_threshold(
    ...
    # Keep tile centers >= safe_radius from the image border so sampled tiles
    # never extend beyond the image boundary (0 = available, 255 = unavailable).
    r = int(safe_radius)
    if r > 0:
        mask_available[:r, :] = 255
        mask_available[-r:, :] = 255
        mask_available[:, :r] = 255
        mask_available[:, -r:] = 255
...
class Stage3SamplingApp(tk.Tk):
    def __init__(self, run_dir: Path):
        ...
        self.title("Stage 3 — CVT sampling")
        ...
        # ---- load stage3 params from parameters.json (same folder as this script)
        ...
        self.panel_left  = self._make_image_panel(mid, "H&E-FG Image")
    ...
    def _load_initial_left(self):
        ...
        if img is None:
            self._set_placeholder(self.panel_left, "missing H&E-FG")
    ...
    def on_sampling_clicked(self):
        ...
        try:
            ...
            if "he0_level_override" in self.stage3 and self.stage3["he0_level_override"] not in [None, "None"]:
                HE0_EXTRACT_LEVEL = int(self.stage3["he0_level_override"])
            else:
                HE0_EXTRACT_LEVEL = choose_tile_level(
                    x=HE0_LEVEL,
            ...
            tile_size_at_extract = float(self.stage3["tile_size"])   # e.g. 600 px at extract level
            ...
            log_event(
                ...
                n_points=int(len(points_xy)),
                he0_level=int(HE0_LEVEL),
                he0_extract_level=int(HE0_EXTRACT_LEVEL),
                tile_size_sampling=float(TILE_SIZE),
                sampled_points_json=str(out_json),
                overlay_path=str(self.run_dir / "3_sampled_overlay.png"),
            )
    ...
    def on_extract_clicked(self):
        ...
        def worker(q):
            ...
            script_dir = Path(__file__).resolve().parent
            from my_utils import cli_params_block
            parameters = cli_params_block()

            HE0_PATH = info["HE0_path"]
            HE_PATH = info["HE_path"]
            HE0_LEVEL = int(info["HE0_level"])
            HE_LEVEL = int(info["HE_level"])
            HE0_TILE_LEVEL_OVERRIDE = parameters["stage3"]["he0_level_override"]
            ...
            if HE0_EXTRACT_LEVEL is None:
                if HE0_TILE_LEVEL_OVERRIDE in [None, "None"]:
                    ...
                    raise ValueError("HE0_TILE_LEVEL_OVERRIDE in parameter.json['stage3'] must be None, 'None', or int")
            ...
            # ------------------------------
            report_stage(4, "Saving H&E tiles")
            ...
            tick(95, f"Saved matched H&E-FG/H&E tiles: {len(matched_keys)}")
