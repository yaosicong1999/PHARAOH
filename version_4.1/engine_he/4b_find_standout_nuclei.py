# ============================================================
# 4b_find_standout_nuclei.py - Stage 4b: Standout nuclei detection
#
# Aligns HE0/HE masks per tile (3-phase), detects isolated nucleus
# pairs, and writes standout_nuclei.json with anchors.
# Usage: python 4b_find_standout_nuclei.py <TILES_DIR>
# ============================================================
import cv2
import numpy as np
...
GOOD_NUCLEI_MIN = 2
MIN_GOOD_TILES = 30          # “pairs>=n” 的 tile 数阈值
...
def he0_aligned_to_original(xA_aligned, yA_aligned, H, W, case_id, he0_mask_scale):
    # aligned(HE0 after apply_orientation) -> original HE0 mask coords
    xA_he0, yA_he0 = inverse_orientation_point(xA_aligned, yA_aligned, H, W, case_id)
    # original HE0 image coords (or “mask level0” coords) by dividing mask_scale
    return float(xA_he0 / he0_mask_scale), float(yA_he0 / he0_mask_scale)
...
def robust_median_scale_from_firstN(
    ...
    scales = []
    use = min(N, len(he0_files))
    print(f"[INFO] Phase-1: coarse on first {use}/{len(he0_files)} tiles to estimate median scale")

    for i in range(use):
        he0_path = he0_files[i]
        fname = os.path.basename(he0_path)
        prefix = fname.replace("_he0_mask.png", "")
...
def load_stage4b_params(base_dir):
    ...
    params = {
        ...
        "phase1": {
            "n_tiles": 50, "ds": 4,
            "scale_min": 1.0, "scale_max": 1.5, "scale_step": 0.05,
            "shift_frac": 0.20, "shift_step_frac": 0.05,
        },
        "phase2": {
            "scale_step": 0.02, "shift_frac": 0.40,
            "shift_step_frac": 0.05, "median_scale_frac": 0.10,
        },
        "phase3": {
            "scale_range_frac": 0.02, "scale_step_frac": 0.005,
            "shift_range_frac": 0.03, "shift_step_frac": 0.01,
        },
    }
    ...
    if params["pairs_to_take_per_tile"] > params["pair_top_k"]:
        print(
            ...
            f"> pair_top_k ({params['pair_top_k']}); output per tile is limited by pair_top_k.",
            flush=True
        )
    return params
...
if __name__ == "__main__":
    ...
    PAIR_TOP_K = int(stage4b_params["pair_top_k"])
    PAIRS_TO_TAKE = int(stage4b_params["pairs_to_take_per_tile"])
    ...
    HE0_MASK_SCALE = mask_scale_data['mask_scale']['he0']
    HE_MASK_SCALE = mask_scale_data['mask_scale']['he']

    he0_files = sorted(
        glob.glob(os.path.join(base_dir, "*_he0_mask.png"))
    )
    total = len(he0_files)
    ...
    for idx, he0_path in enumerate(he0_files, 1):
        ...
        if not os.path.exists(he_path):
            ...
            continue
        print(f"\n=== Processing {prefix} ({idx}/{len(he0_files)}) ===")
        nucleus_id = 0
        # -------- read masks --------
        maskA = read_mask(he0_path)
        maskA = apply_orientation_to_tile(maskA, case_id)
        ...
        for nucleus_id, p in enumerate(pairs):  # 这里你要保留几个pair，后面再切也行
            ...
            xA_img, yA_img = he0_aligned_to_original(
            ...
            pairs_mapped.append({
                ...
                "aligned": {
                    "he0": [float(xA_aligned), float(yA_aligned)],
                    "he": [float(xB_aligned), float(yB_aligned)],
                },
                "original": {
                    "he0": [float(xA_img), float(yA_img)],
    ...
    if mode == "pairs>=n":
        for t in selected_tiles:
            prefix = t["tile"]
            take = t["pairs_mapped"][:PAIRS_TO_TAKE]
            for pm in take:
                final_records.append({
                    ...
                    "meta": {  # ✅ 把 case_id & mask_scale 一并写进去，方便 downstream
                        "case_id": int(case_id),
                        "mask_scale": {"he0": float(HE0_MASK_SCALE), "he": float(HE_MASK_SCALE)},
                    }
                })
        ...
        for t in selected_tiles:
            ...
            xA_img, yA_img = he0_aligned_to_original(cx, cy, H, W, case_id, HE0_MASK_SCALE)
            xB_img, yB_img = he_aligned_to_original(
                cx, cy,
                final, H, W,
                paste_x0, paste_y0,
    ...
    for rec in final_records:
        ...
        # raw image paths
        he0_path = os.path.join(base_dir, f"{tile}_he0.png")
        he_path = os.path.join(base_dir, f"{tile}_he.png")

        if not os.path.exists(he0_path):
            print(f"[WARN] Missing HE0 raw tile: {he0_path}")
            continue
        if not os.path.exists(he_path):
