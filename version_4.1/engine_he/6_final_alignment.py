# ============================================================
# 6_final_alignment.py - Stage 6: Final alignment viewer
#
# Displays HE0, HE, HE0 warped onto HE, and transformed cell
# centroids using the final registration result. Also exports a
# warped HE0 OME-TIFF in HE space.
# Usage: python 6_final_alignment.py <RUN_DIR>
# ============================================================
import os
import sys
import json
...
class FinalAlignmentApp(tk.Tk):
    def __init__(self, run_dir: Path):
        ...
        self.cache = {
            "he0_base": self.run_dir / f"6_cache_he0_base_L{self.display_level}.png",
            "he_base": self.run_dir / f"6_cache_he_base_L{self.display_level}.png",

            "he0_kp": self.run_dir / f"6_cache_he0_kp_L{self.display_level}.png",
        ...
        print("[DEBUG] H&E   image shape:", self.he_rgb.shape, flush=True)
    ...
    def refresh_images_after_alignment(self):
        ...
        if he0_kp is None:
            ...
            he0_kp = draw_points(self.he0_rgb, pts, color=(0, 255, 0), r=6, name="H&E-FG nuclei")
        ...
        if he_kp is None:
            ...
            he_kp = draw_points(self.he_rgb, pts, color=(0, 255, 0), r=6, name="H&E nuclei")
    ...
    def load_cell_data(self):
        ...
        try:
            ...
            print("[DEBUG] H&E image level2 shape:", self.he_rgb.shape, flush=True)
    ...
    def on_panel_click(self, idx: int):
        ...
        if idx == 0:
            if self.alignment_loaded and self.he0_pts0 is not None:
                pts = self.he0_pts0 / self.display_scale
                img = draw_points(self.he0_rgb, pts, color=(0, 255, 0), r=6, name="H&E-FG nuclei (large)")
            else:
                img = self.he0_rgb.copy()
            img = apply_orientation_case(img, self.case_id)
            self.show_large_view("H&E-FG", img)
            return
        ...
        if idx == 1:
            if self.alignment_loaded and self.he_pts0 is not None:
                pts = self.he_pts0 / self.display_scale
                img = draw_points(self.he_rgb, pts, color=(0, 255, 0), r=6, name="H&E nuclei (large)")
            else:
