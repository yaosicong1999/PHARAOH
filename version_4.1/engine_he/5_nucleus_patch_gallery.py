# ============================================================
# 5_nucleus_patch_gallery.py - Stage 5: Patch gallery & final transform
#
# Review/edit nucleus centroid pairs, drop bad pairs, then estimate
# the final HE0->HE transform (affine / homography / TPS) into
# he0_to_he_homography_level0.json.
# Usage: python 5_nucleus_patch_gallery.py <RUN_DIR>
# ============================================================
...
def load_stage5_transform_mode(run_dir, default="tps"):
    """Read stage5.transform_mode from <run_dir>/../parameters.json (affine/homography/tps)."""
    valid = {"homography", "affine", "tps"}
    from my_utils import cli_params_block
    data = cli_params_block()
    mode = str(data.get("stage5", {}).get("transform_mode", default)).strip().lower()
    if mode == "local_tps":
        mode = "tps"   # this gallery supports affine/homography/tps only
    if mode not in valid:
        print(f"[WARN] Invalid stage5.transform_mode={mode}; using {default}.", flush=True)
        return default
    return mode
...
def show_nucleus_patch_in_memory(
    ...
    transform_mode = load_stage5_transform_mode(run_dir)
    print(f"[INFO] stage5 transform_mode = {transform_mode}", flush=True)
    ...
    root.title("Stage 5: H&E-FG & H&E Patch Gallery")
    ...
    def calculate_perspective_ransac(transform_type="tps"):
        ...
        try:
            ...
            log_event(run_dir, "system_ready_calculate_transformation",
                      transform_type=transform_type, output_file=str(out_path))
    ...
    btn_calc = ttk.Button(
        ...
        command=lambda: calculate_perspective_ransac(transform_mode)
    )
...
def main():
    if len(sys.argv) < 2:
        raise RuntimeError("Usage: python 5_nucleus_patch_gallery.py <patches_folder>")
