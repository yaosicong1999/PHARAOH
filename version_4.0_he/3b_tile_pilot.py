# ============================================================
# 3b_tile_pilot.py - Stage 3 (pilot): Pilot tile extraction
#
# Picks a farthest-point subset of sampled centroids, extracts
# pilot HE0 + HE tiles into <RUN_DIR>/pilot_tiles/, then launches
# 3c for visual inspection.
# Usage: python 3b_tile_pilot.py <RUN_DIR>
# ============================================================
import os
import sys
import json
...
def load_stage3_params(script_path: Path):
    ...
    stage3["he_tile_margin_ratio"] = float(stage3.get("he_tile_margin_ratio", 0.1))
    return stage3
...
def main():
    ...
    if not sampled_path.exists():
        raise FileNotFoundError(f"missing {sampled_path} (run Stage3 sampling first)")
    ...
    HE0_TILE_LEVEL_OVERRIDE = parameters["stage3"].get("he0_level_override", "None")
    HE_TILE_LEVEL_OVERRIDE = parameters["stage3"].get("he_level_override", "None")
