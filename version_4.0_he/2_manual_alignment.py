    ...
    def run(self):
        try:
            ...
            if src is None or dst is None:
                self.failed.emit("could not read HE0/HE low-level images")
                return
    ...
    def on_save(self):
        ...
        try:
            ...
            def _compute_H_gui2he():
                ...
                src_gui, dst_he = self.view.get_corners_he0_to_he()   # (4,2)
            ...
            he0_mask_bgr = cv2.imread(str(run_dir / "1_confirmed_he0_dense_mask.png"))
            warped_he0_mask = warp_mask(
                he0_mask_bgr,
                H_gui2he_mask,
                (he_mask_bgr.shape[1], he_mask_bgr.shape[0])
            )
            he_rgba = mask_to_rgba(he_mask_bgr, color_rgb=(255, 0, 0), alpha=0.5)
            he0_rgba = mask_to_rgba(warped_he0_mask, color_rgb=(0, 0, 255), alpha=0.5)
            out = overlay_rgba_on_bgr(he_mask_bgr, he_rgba)
            out = overlay_rgba_on_bgr(out, he0_rgba)
...
if __name__ == "__main__":
    ...
    missing = [p for p in [he_mask_path, he0_mask_path, he_orig_path, he0_orig_path] if not p.exists()]
    if missing:
